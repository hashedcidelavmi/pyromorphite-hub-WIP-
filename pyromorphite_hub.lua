...
toggleButton.MouseButton1Click:Connect(function()
	if dragDistance < 5 then
		if open then
			Title.Text = ""
			local tween = TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Size = UDim2.new(0, 0, 0, 0)})
			tween:Play()
			...
			TweenService:Create(Main, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, 380, 0, 280), Position = UDim2.new(0.5, -190, 0.5, -140)}):Play()
...
local function createVisualizer()
	...
	for i = 1, SEGMENTS do
		...
		wedge.Transparency = 0.25; wedge.CastShadow = false; wedge.Parent = Workspace
...
RunService.RenderStepped:Connect(function()
	if visualEnabled then for _, p in ipairs(Players:GetPlayers()) do updateVisualizer(p) end end
end)
		...
		task.spawn(function()
			...
			if char then
				for _, part in ipairs(char:GetDescendants()) do
					if part:IsA("BasePart") and (part.Name == "HumanoidRootPart" or part.Name == "UpperTorso" or part.Name == "LowerTorso" or part.Name == "Torso" or part.Name == "Head") then part.CanCollide = true end
				end
			end
		end)
	end
...
local function CreateEmbeddedFlingGUI()
	...
	FMain.Size = UDim2.new(0, 320, 0, 390); FMain.Position = UDim2.new(0.5, -160, 0.5, -195)
	...
	TitleFill.Size = UDim2.new(1, 0, 0, 10); TitleFill.Position = UDim2.new(0, 0, 1, -10); TitleFill.BackgroundColor3 = Black; TitleFill.BorderSizePixel = 0
	local FTitle = Instance.new("TextLabel", TitleBar)
	FTitle.Size = UDim2.new(1, -35, 1, 0); FTitle.BackgroundTransparency = 1; FTitle.Text = "Pyromorphite Fling GUI"; FTitle.TextColor3 = EmeraldGreen; FTitle.Font = Enum.Font.Arcade; FTitle.TextSize = 16
	local CloseButton = Instance.new("TextButton", TitleBar)
	CloseButton.Position = UDim2.new(1, -30, 0, 2); CloseButton.Size = UDim2.new(0, 26, 0, 26)
	CloseButton.BackgroundColor3 = Black; CloseButton.Text = "X"; CloseButton.TextColor3 = ActiveRed; CloseButton.Font = Enum.Font.Arcade; CloseButton.TextSize = 16; CloseButton.ZIndex = 5
	...
	StatusLabel.Position = UDim2.new(0, 10, 0, 40); StatusLabel.Size = UDim2.new(1, -20, 0, 25); StatusLabel.BackgroundTransparency = 1; StatusLabel.Text = "Select targets to fling"; StatusLabel.TextColor3 = EmeraldGreen; StatusLabel.Font = Enum.Font.Arcade; StatusLabel.TextSize = 14
	...
	SelectAllBtn.Position = UDim2.new(0, 10, 0, 260); SelectAllBtn.Size = UDim2.new(0.5, -15, 0, 30); SelectAllBtn.BackgroundColor3 = Black; SelectAllBtn.Text = "SELECT ALL"; SelectAllBtn.TextColor3 = EmeraldGreen; SelectAllBtn.Font = Enum.Font.Arcade; SelectAllBtn.TextSize = 12
	...
	ClearAllBtn.Position = UDim2.new(0.5, 5, 0, 260); ClearAllBtn.Size = UDim2.new(0.5, -15, 0, 30); ClearAllBtn.BackgroundColor3 = Black; ClearAllBtn.Text = "CLEAR ALL"; ClearAllBtn.TextColor3 = EmeraldGreen; ClearAllBtn.Font = Enum.Font.Arcade; ClearAllBtn.TextSize = 12
	...
	StartButton.Position = UDim2.new(0, 10, 0, 305); StartButton.Size = UDim2.new(0.5, -15, 0, 40); StartButton.BackgroundColor3 = Black; StartButton.Text = "START FLING"; StartButton.TextColor3 = EmeraldGreen; StartButton.Font = Enum.Font.Arcade; StartButton.TextSize = 14
	...
	StopButton.Position = UDim2.new(0.5, 5, 0, 305); StopButton.Size = UDim2.new(0.5, -15, 0, 40); StopButton.BackgroundColor3 = Black; StopButton.Text = "STOP FLING"; StopButton.TextColor3 = EmeraldGreen; StopButton.Font = Enum.Font.Arcade; StopButton.TextSize = 14
	...
	local function Message(t, txt, dur) pcall(function() StarterGui:SetCore("SendNotification", {Title = t, Text = txt, Duration = dur or 3}) end) end
	...
	local function RefreshPlayerList()
		...
		for _, p in ipairs(Players:GetPlayers()) do
			if p ~= Player then
				...
				ClickArea.Size = UDim2.new(1, 0, 1, 0); ClickArea.BackgroundTransparency = 1; ClickArea.Text = "  " .. p.Name; ClickArea.TextColor3 = EmeraldGreen; ClickArea.TextXAlignment = Enum.TextXAlignment.Left; ClickArea.Font = Enum.Font.Arcade
				local Checkmark = Instance.new("TextLabel", PlayerEntry)
				Checkmark.Size = UDim2.new(0, 30, 1, 0); Checkmark.Position = UDim2.new(1, -30, 0, 0); Checkmark.BackgroundTransparency = 1; Checkmark.Text = "[X]"; Checkmark.TextColor3 = ActiveRed; Checkmark.Visible = SelectedTargets[p.Name] ~= nil; Checkmark.Font = Enum.Font.Arcade
				CheckboxObjects[p.Name] = Checkmark
				ClickArea.MouseButton1Click:Connect(function()
					if SelectedTargets[p.Name] then SelectedTargets[p.Name] = nil; Checkmark.Visible = false else SelectedTargets[p.Name] = p; Checkmark.Visible = true end
					UpdateStatus()
				end)
	...
	SelectAllBtn.MouseButton1Click:Connect(function()
		for _, p in ipairs(Players:GetPlayers()) do if p ~= Player then SelectedTargets[p.Name] = p; if CheckboxObjects[p.Name] then CheckboxObjects[p.Name].Visible = true end end end
		UpdateStatus()
	end)
	...
	local function SkidFling(TargetPlayer)
		...
		if getgenv().OldPos then RootPart.CFrame = getgenv().OldPos end
	end
	StartButton.MouseButton1Click:Connect(function()
		if FlingActive then return end FlingActive = true; UpdateStatus()
		task.spawn(function()
			while FlingActive do
				for name, p in pairs(SelectedTargets) do if p and p.Parent and FlingActive then SkidFling(p) task.wait(0.1) end end
				task.wait(0.5)
			end
		end)
	end)
	StopButton.MouseButton1Click:Connect(function() FlingActive = false; UpdateStatus() end)
	CloseButton.MouseButton1Click:Connect(function() FlingActive = false; FlingGui:Destroy() end)
	Players.PlayerAdd